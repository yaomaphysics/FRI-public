...
def _ts_str(m, font=None):
    """WL typeset expression for a regge mode string (LaTeX-style):
    'C13' -> C_13, 'C1^2C13' -> C_1^2 C_13, 'S^2C24' -> S^2 C_24,
    'C1\u221eC13' -> C_1^\u221e C_13;  G / sH / H italic."""
    font = font or ATLAS_FONT
...
def ts_mode(m, font=None):
    """WL typeset expression for a regge mode string."""
...
_ATLAS_MODE_ORDER = {'Green': 0, 'DarkGreen': 1, 'Magenta': 2, 'Red': 3,
                     'Orange': 4, 'Pink': 5, 'D12': 6, 'D6': 7,
                     'Black': 8, 'DarkYellow': 9, 'DarkRed': 10, 'Blue': 11}


def atlas_mode_key(m):
    """Order of the per-mode lines: the module's colour groups first
    (C13 family, C24 family, soft), H last; the name breaks ties."""
