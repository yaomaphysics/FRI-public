...
# ============================ enumerator ============================
def cut_options(edges, verts, roots, forbid=frozenset()):
    # Connected subsets containing roots (empty included).
    roots = set(roots)
    rest = sorted(set(verts) - roots - set(forbid))
    out = [frozenset()]
    for r in range(len(rest) + 1):
        for sub in combinations(rest, r):
            S = frozenset(roots | set(sub))
    ...
    for r in range(len(cand) + 1):
        for sub in combinations(cand, r):
            ...
            for c in groups.values():
                in2, in3 = v2 in c, v3 in c
                if k1:
                    if not (in2 and in3): ok = False; break
                else:
                    if not (in2 or in3): ok = False; break
            if ok: out.append(S)
    return out
...
def _refine_towers23(edges, verts, root, base, forbid, levels):
    # Nested refined-cut towers: level i is inside level i-1, all within base; empty levels = no refined cut.
    if levels <= 0:
        return [()]
    out = []
    for s1 in refined_opts23(edges, verts, root, base, forbid):
        if s1:
            for rest in _refine_towers23(edges, verts, root, s1, forbid,
                                         levels - 1):
                out.append((s1,) + rest)
        else:
            out.append(tuple([frozenset()] * levels))
    return out
...
def enumerate_regions(edges, verts, ext_attach, ext_mode, restrict=True, verbose=False):
    ...
    def _mf(leg):
        x = ext_mode[leg][2]           # wide: x = m_i; pair: x = m_i - 1
        if x == INF: return None
        return x + 1 if leg in ('p2', 'p3') else x   # all five m_i
    ...
    if not _f:
        ...
        def _top(leg):
            x = ext_mode[leg][2]       # M_i = min{m_i, m}
            if x == INF: return msf
    ...
    forbid2, forbid3 = extvs - {v2v}, extvs - {v3v}
    if USE_C23_SPLIT:
        O23 = cut_options_split(edges, verts, v2v, v3v,
                                forbid=extvs - R23, k1=_allinf)
    else:
        O23 = cut_options(edges, verts, R23, forbid=extvs - R23)
    O1 = cut_options(edges, verts, R1, forbid=extvs - R1)
    O4 = cut_options(edges, verts, R4, forbid=extvs - R4)
    O5 = cut_options(edges, verts, R5, forbid=extvs - R5)
    stats = {}
    survivors = []
    seen = set()
    vm_seen = set()
    ...
    for c23 in O23:
        ...
        T3 = _refine_towers23(edges, verts, v3v, c23, forbid3, _L3)
        for t2 in T2:
            for t3 in T3:
                r2 = t2[0] if t2 else frozenset()
                r2b = t2[1] if len(t2) > 1 else frozenset()
                r3 = t3[0] if t3 else frozenset()
                r3b = t3[1] if len(t3) > 1 else frozenset()
                for c1 in O1:
                    T1 = _refine_towers23(edges, verts, v1v, c1, forbid1, _L1)
                    for t1 in T1:
                        r1 = t1[0] if t1 else frozenset()
                        for c4 in O4:
                            T4 = _refine_towers23(edges, verts, v4v, c4, forbid4, _L4)
                            for t4 in T4:
                                r4 = t4[0] if t4 else frozenset()
                                for c5 in O5:
                                    T5 = _refine_towers23(edges, verts, v5v, c5, forbid5, _L5)
                                    for t5 in T5:
                                        r5 = t5[0] if t5 else frozenset()
                                        total += 1
                                        cuts = {'C23': set(c23), 'C1': set(c1), 'C4': set(c4),
                                                'C5': set(c5), 'C1R1': set(r1), 'C4R1': set(r4),
                                                'C5R1': set(r5), 'C2R1': set(r2), 'C3R1': set(r3),
                                                'C2R2': set(r2b), 'C3R2': set(r3b)}
                                        # H-vertex prune — at least one vertex must lie OUTSIDE all cuts
                                        # (the single strongest filter of the sweep)
                                        ...
                                        if res is None:
                                            ...
                                            if verbose: print('  REJ', cuts, '->', why)
                                            continue
                                        em, vm = res
                                        if not momentum_ok(edges, verts, ext_attach, ext_mode, em, vm):
                                            stats['momentum'] = stats.get('momentum', 0) + 1; continue
                                        ok, info = jets_ok(edges, verts, vm, em, ext_attach)
                                        if not ok:
                                        ...
                                        if not island_ok(edges, verts, em, vm):
                                            stats['island'] = stats.get('island', 0) + 1; continue
                                        dbg = {}
                                        if not ir_ok(edges, verts, em, vm, ext_attach, ext_mode, dbg):
                                            stats['IR'] = stats.get('IR', 0) + 1
                                            if verbose: print('  REJ', cuts, '-> IR', dbg.get('unconfirmed', ''))
                                            continue
                                        vec = tuple(-V(m) if V(m) != INF else 'inf' for m in em) + (1,)
                                        key = (vec, tuple(sorted(vm.items())))
                                        if key in seen: continue
                                        seen.add(key)
                                        survivors.append((vec, cuts, em, vm))
    return survivors, stats, total

# ============================ example ============================
# Built-in example: the 5-point graph "Frog" (2 loops, 6 edges), external
# momenta p1..p5 attached at vertices 1..5; the five external-virtuality
# patterns k0..k4 are defined in kin23.py.
#
# Run:  python3 fri23.py [k0..k4] [-v]
FROG_VERTS = [1, 2, 3, 4, 5]
FROG_EDGES = [(1, 3), (1, 5), (2, 3), (2, 5), (3, 4), (4, 5)]
FROG_EXT_ATTACH = {'p1': 1, 'p2': 2, 'p3': 3, 'p4': 4, 'p5': 5}


def run_example(kind='k1', verbose=False):
    import kin23
    ext_mode = kin23.ext_modes(kind)
    t0 = time.time()
    surv, stats, total = enumerate_regions(FROG_EDGES, FROG_VERTS,
                                           FROG_EXT_ATTACH, ext_mode,
                                           verbose=verbose)
    dt = time.time() - t0
    vecs = sorted(s[0] for s in surv)
    print(f'== Frog ({kind}) ==  combos={total}  FRI={len(vecs)} regions  ({dt:.1f}s)')
    for i, vec in enumerate(vecs, 1):
        print(f'  R{i:3d}: {vec}')
    if verbose:
        for vec, cuts, em, vm in surv:
            print('  ---', vec)
            print('      cuts:', {k: sorted(v) for k, v in cuts.items() if v})
            print('      em:', [name(x) for x in em])
            print('      vm:', {v: name(vm[v]) for v in sorted(vm)})
    return vecs


if __name__ == '__main__':
    args = [a for a in sys.argv[1:] if not a.startswith('-')]
    kind = args[0] if args else 'k1'
    import kin23
    if kind not in kin23.KIN_ORDER:
        print(f'usage: python3 fri23.py [{"|".join(kin23.KIN_ORDER)}] [-v]')
        sys.exit(1)
    run_example(kind, verbose=('-v' in sys.argv))
